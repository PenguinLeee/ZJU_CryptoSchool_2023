# Lattice 3-4: LWE\SIS for PKE\SIG\FHE
...
又根据LHL，我们无法区分$rA$和均匀随机向量、$rz_2$和随机向量。
...
$\operatorname{KeyGen}$：$(A, T)$——产生陷门。$pk = A, sk=T$。

$\operatorname{Sign}(m)$。计算$r_m$，使得$A \cdot r_m = H(m)$。其中，$H(m)$是$m$的哈希值。注意，需要有私钥$T$的参与才能完成签名。
...
根据定义，攻击者可以通过查询RO来获取消息对应的哈希值。这意味着，RO需要维护一张表，对于给定的输入$x$，RO要能查到对应的$r$，并且返回$H(x) = Ar$。
...
我们观察上述的乘法操作，可以发现：
1. $G^{-1}$大概将噪声扩大了$\sqrt{n}$倍。
2. 假如给定密文$C_1, C_2$，它们的噪声界分别为$b_1 > b_2$，那么你计算$C_1C_2$和计算$C_2C_1$的噪声界存在差别，因为乘法的先后次序不同，引入的噪声是不一样的。
...
由于时间不够，课件里主要从high level讲了Bootstrapping。这里，我们引用<a href=https://blog.csdn.net/weixin_43466027/article/details/126013689>本人的这篇博客</a>给出的关于自举的简介：
